...
```{r}
library(readxl)
Chile <- read_excel("~/maestria analitica de datos/Estadistica/1. ChileTaller.xlsx", 
...
BPregion <- barplot(prop.table(table(Chile$region)),col=c("orange","blue","green","red","purple"),legend.text=c("Center","Metropolitan","North","South","City of Santiago"),ylim=c(0,0.8),xlim=c(0,11),main="Frecuencias relativas de participaciÃ³n por regiÃ³n",ylab ="Frecuencias Relativas")

```

**Para el caso de la variable Region se puede identificar que las region de City of Santiago (n= 960) es la que tuvo mayor participación en las votaciones, seguida por la región sur (n=718) y la región center (n=600)**
...
BPsex <- barplot(prop.table(table(Chile$sex)),col=c("orange","blue"),legend.text=c("Femenino","Masculino"),ylim=c(0,0.8),xlim=c(0,3),main="Frecuencias relativas de participaciÃ³n en votaciones por sexo",ylab ="Frecuencias Relativas")
```

**Para el caso de la variable cualitativa sexo pudemos observar que la proporsión de votantes hombres y mujeres es muy similar con una leve proporsión mayor en participación de mujeres**
...
BPEdu <- barplot(prop.table(table(Chile$education)),col=c("orange","blue","green","purple"),legend.text=c("Not answered","Primary","Post Secundary","Secondary"),ylim=c(0,0.8),xlim=c(0,6),main="Frecuencias relativas de participaciÃ³n en votaciones por nivel educativo",ylab ="Frecuencias Relativas")

```

**para el caso de la variable educación se puede identificar que la proporsion de votantes es mayor en personas con un nivel educativo de "primaria" y "secundaria", por su parte "post secundaria" cuanta con una muy baja participación en las elecciones de chile**
...
**Respecto a la variable relacionada con la intensiÃ³n de voto por el presidente "pinochet" se encuentra que las personas que votarÃ¡n SI (33%) son muy cercanas a las personas que votarÃ¡n NO (32%)**
...
**Conclusiones**
De acuerdo a la anterior tabla de contingencia encontramos que el mayor valor chi cuadrado se encuentra entre un nivel de estudio de primaria (P) y la intensión de votar NO por pinochet como presidente. Por otro lado se evidencia que dicha relacion entre la intensión de votar NO por pinochet como presidente es alta en personas con un nivel educativo de PostSecundaria (PS).

En otro de los cruces de variable en donde encontramos un chi cuadrado superior es en la intensión de voto de respaldar a Pinochet con el SI en personas con un nivel educativo de primaria (P) 
...
```{r}
library(readxl)
Nutrition <- read_excel("~/maestria analitica de datos/Estadistica/2. nutrition_elderly.xlsx", 
